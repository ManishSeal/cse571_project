...
class RobotActionsServer:
    ...
    def __init__(self, object_dict, root_path, headless, random_seed=10):
        ...
        self.action_publisher = rospy.Publisher(
            "/actions", String, queue_size=10)
        self.status_publisher = rospy.Publisher(
            "/status", String, queue_size=10)
        self.random_seed = random_seed
        self.current_state = self.generate_init_state()
        self.action_config = self.load_action_config(
            root_path + '/action_config.json')
        ...
        rospy.Service('get_possible_actions', GetPossibleActions,
                      self.get_possible_actions)
        rospy.Service('get_possible_states', GetPossibleStates,
                      self.get_possible_states)
        rospy.Service('get_reward', GetReward, self.get_reward)
        rospy.Service('is_terminal_state', IsTerminalState,
                      self.is_terminal_state_handler)
        rospy.Service('get_current_state', GetInitialState,
                      self.get_current_state)
        rospy.Service('get_unplaced_books', UnplacedBooks,
                      self.get_unplaced_books)
        rospy.Service('update_currentstate_objectdict',
                      UpdatedTuple, self.update_currentstate_objectdict)
    ...
    def generate_init_state(self):
        ...
        for book in self.object_dict["books"]:
            self.object_dict["books"][book]["placed"] = False
            state[book] = {
        ...
        state['basket'] = self.object_dict['basket']
        return state
    ...
    def get_unplaced_books(self, req):
        ...
        # returning the whole dict, as need to check whether any location is occupied right now or not by another book
        for book_name, val_dict in self.object_dict["books"].items():
            if val_dict['placed'] is False:
                new_dict = dict(val_dict)
                new_dict["book_name"] = book_name
                result.append(new_dict)

        result = json.dumps(self.object_dict)
    ...
    def remove_edge(self, book_name):
        ...
        try:
            remove_edge = rospy.ServiceProxy(
                'remove_blocked_edge', RemoveBlockedEdgeMsg)
        ...
        for possible_action in self.action_config[action]['possibilities']:
            ...
            if possible_action == "noaction":
                next_states[state_key] = (
                    state, self.action_config[action]['possibilities'][possible_action])
                continue
            ...
            calling_function = "self.{}({})".format(
                self.action_config[possible_action]['function'], ','.join(calling_params))
            success, next_state = eval(calling_function)

            next_states[state_key] = (
                next_state, self.action_config[action]['possibilities'][possible_action])
    ...
    def execute_action(self, req):
        ...
        print "possibilities: ", self.action_config[action]['possibilities'].keys(
        )
        print "probabilities: ", self.action_config[action]['possibilities'].values(
        )
        ...
        calling_function = "self.{}({})".format(
            self.action_config[chosen_action]['function'], ','.join(calling_params))
        success, next_state = eval(calling_function)

        # Update state
        # We lose the spawnned books info if we do this
        # self.current_state = copy.deepcopy(next_state)

        for state_vars in next_state:
        ...
        if book_name in self.object_dict["books"] and bin_name in self.object_dict["bins"]:
            ...
            if (robot_state[0], robot_state[1]) in self.object_dict["bins"][bin_name]["load_loc"]:
                ...
                if self.object_dict["books"][book_name]["size"] == self.object_dict["bins"][bin_name]["size"] and \
                    ...
                    if simulation:
                        goal_loc = list(
                            self.object_dict["bins"][bin_name]["loc"])
                ...
                if current_state['basket'] == 'empty':
                    ...
                    if simulation:
                        self.change_gazebo_state(
                            book_name, list(robot_state[:2])+[2])
                        rospy.Rate(1).sleep()
    ...
    def execute_TurnCW(self, current_state, simulation=False):
        ...
        new_orientation = self.direction_list[(
            self.direction_list.index(current_orientation) + 1) % 4]
